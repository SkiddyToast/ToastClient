...
object RenderUtil : RenderExtensions {
    ...
    fun drawBox(box: Box, color: ToastColor) {
        ...
        tessellator.buffer.begin(VertexFormat.DrawMode.LINE_STRIP, VertexFormats.POSITION_COLOR)
        box(
            matrix,
            tessellator.buffer,
            box,
    ...
    fun drawGuiItem(x: Double, y: Double, z: Double, offX: Double, offY: Double, scale: Double, item: ItemStack) {
        ...
        GL11.glDepthFunc(GL11.GL_ALWAYS)
        mc.itemRenderer.renderItem(item, ModelTransformation.Mode.GUI, 0xF000F0, OverlayTexture.DEFAULT_UV, matrix, vertex, 0)
        vertex.draw()
